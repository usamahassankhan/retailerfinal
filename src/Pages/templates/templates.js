...
export default function EnhancedTable() {
  ...
  console.log(
    "sdas",
    tabledata?.map((a) => a.team1.plantemplateid)
  );
  ...
  const renderRows = () => {
    ...
    if (Boolean(tabledata?.length)) {
      for (let i in tabledata) {
        arr.push(
          createData(
            tabledata[i]?.team1.id,
            tabledata[i]?.team1.plantemplatename,
            tabledata[i]?.team1.plantemplateid,
            tabledata[i]?.team1.targetSpend,
            tabledata[i]?.team1.cashbackoffer,
            tabledata[i]?.team1.cashbackoffer,
            tabledata[i]?.team1.maxshoppers
          )
        );
      }
  ...
  const useTemplate = async () => {
    ...
    await db
      ...
      .doc(selectedrow.PlanTemplateid)
      .set({
        id: selectedrow.id,
  ...
  console.log(isSelected, "dsadasdas");
  // const isSelected = () => {
  ...
  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, renderRows().length - page * rowsPerPage);
    ...
    <div className={classes.root} className="wao">
          ...
          <TableContainer>
            {renderRows().length > 0 ? (
                ...
                <TableBody>
                  {stableSort(renderRows(), getComparator(order, orderBy))
                    ...
                    .map((row, index) => {
                      ...
                      console.log(isItemSelected, "dasdas");
                      return (
                        <TableRow
                          hover
                          onClick={(event) =>
                            handleClick(event, row.PlanTemplateid, row)
                          }
                          role="checkbox"
              ...
              <div style={{ textAlign: "center" }}>
                <h1>there is no template please create a new one</h1>
              </div>
            )}
          </TableContainer>
          <TablePagination
      ...
      <div className="templatebtn">
        ...
        {selected.length > 0 ? (
          ...
          <button disabled onClick={useTemplate}>
            Use Template{" "}
          </button>
        )}
      </div>
      <div className="templatelast">
        <p>
          {" "}
          <strong>
            Don’t see a template that works for you? Create a new one
          </strong>
        </p>
        <button onClick={() => history.push("templateform")}>
          Create New Template{" "}
        </button>
      </div>
      <Modal className={classes.modal} open={open} onClose={handleClose}>
        <div className={classes.paper1}>
          ...
          <div style={{ display: "flex", justifyContent: "space-evenly" }}>
            <button className="modalbtn" onClick={deletetemplate}>
              Yes
            </button>
            <button className="modalbtn" onClick={() => setOpen(false)}>
              No
            </button>
          </div>
        </div>
